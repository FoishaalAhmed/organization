...
@section('content')
    ...
    <div class="row news-dtl" style="text-align: center; margin: auto; ">
      <div class="col-md-9 col-sm-9 col-xs-12">
        <h1 style="text-align: left;">{{$news->title}}</h1>
        <img src="{{asset($news->photo)}}" style="width: 100%;" alt="">
        <p style="text-align: left;"><small>{{date('d M, Y', strtotime($news->date))}}</small></p>
        <div style="text-align: justify;padding-left: 10px;font-size: 18px;margin-right: 10px;"> {!! $news->detail !!}</div>
      ...
      <div class="col-md-3 col-sm-9 col-xs-12 ">
        ...
        <div class="">
          ...
          <div class="" style="border-bottom: 2px solid red;margin-bottom: 5px; ">
            <a href="{{route('news.detail', [$news->id,  $news->slug])}}">
            <img src="{{asset($news->photo)}}" style="width: 100%; height: 220px;" alt="">
            <p style="text-align: left;margin-top: 5px; margin-bottom: 5px;">{{$news->title}}</p>
            </a>
          </div>
          @endforeach
        </div>
